...
void Enguin::Draw(float x1, float y1, float x2, float y2, float x3, float y3)
{
	Resource(x1, y1, x2, y2, x3, y3);
}

//CompileShader関数
IDxcBlob* Enguin::CompileShader
(
...
void Enguin::CreatePSO()
{
	...
	graphicsPipelineStateDesc_.PrimitiveTopologyType = D3D12_PRIMITIVE_TOPOLOGY_TYPE_Enguin;
...
void Enguin::Resource(float x1, float y1, float x2, float y2, float x3, float y3)
{
	indexVertex_ = indexEnguin_ * kVertexCountEnguin;

	//頂点リソースにデータを書き込む
	Vector4* vertexData = nullptr;
	//書き込むためのアドレスを取得
	vertexResource_->Map(0, nullptr, reinterpret_cast<void**>(&vertexData[indexEnguin_]));
	...
	//左下
	vertexData[indexEnguin_*3] = { x1,y1,0.0f,2.5f };
	//上
	vertexData[indexEnguin_*3+1] = { x2,y2,0.0f,2.5f };
	//右下
	vertexData[indexEnguin_*3+2] = { x3,y3,0.0f,2.5f };
	...
	//direct_->GetCommandList()->IASetPrimitiveTopology(D3D_PRIMITIVE_TOPOLOGY_EnguinLIST);
	////描画！(DrawCall/ドローコール)。3頂点で1つのインスタンス。
	//direct_->GetCommandList()->DrawInstanced(kVertexCountEnguin, 1, static_cast<UINT>(indexVertex_), 0);
